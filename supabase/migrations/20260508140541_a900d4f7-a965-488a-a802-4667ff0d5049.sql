...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      -- Custom (hr) role with explicit role_features:
      --   • feature in role_features → granted
      --   • else fall back to employee defaults so the user keeps their personal "Me" features
      WHEN v_role = 'hr'
        AND v_role_features IS NOT NULL
        AND cardinality(v_role_features) > 0
        AND (fd.feature_key = ANY(v_role_features))
      ...
      WHEN v_role = 'hr'
        AND v_role_features IS NOT NULL
        AND cardinality(v_role_features) > 0
        AND 'employee' = ANY(fd.default_enabled_for_roles)
        THEN 'edit'::public.feature_access_level
      WHEN v_role = 'hr'
        AND v_role_features IS NOT NULL
        AND cardinality(v_role_features) > 0
